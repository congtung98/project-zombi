...
> Cập nhật: **2026-09-26**, hoàn thành **G0 của kế hoạch đồ họa** (docs/Graphics_Improvement_Implementation_Plan.md: audit, world `graphics-lab`, log từng frame, script chụp/đo baseline). Lộ trình đồ họa đã chốt: G0 → G1 vật liệu/texture → G2 module kiến trúc → G3a đồ đạc / G3b cụm trang trí + biến thể → G4 ánh sáng/tiếp xúc/cảnh quan → G5/G6 editor, hiệu năng, quality tier, rollout. M11c-2 đã commit (3674656). Làm **từng sprint**, dừng sau mỗi sprint để người dùng kiểm tra và commit.
...
> Đọc file này, **docs/Graphics_Improvement_Implementation_Plan.md**, **docs/graphics-g0.md**, README.md, toàn bộ Zombie_Outbreak_Phase_2_Plan.md, docs/phase2-s1.md … phase2-s5.md, docs/phase2-vision.md, docs/phase2-lighting.md, docs/refactor-r0-r2.md, docs/Map_Editor_Implementation_Plan.md, docs/map-content-format.md, docs/map-editor-m1-m2.md, docs/refactor-r3b.md, docs/map-editor-m3.md, docs/map-editor-m4.md, docs/map-editor-m5.md, **docs/map-editor-m6.md**, **docs/map-editor-m7.md**, **docs/map-editor-m8.md**, **docs/map-editor-m9.md**, **docs/world-menu.md**, **docs/map-editor-m10.md**, **docs/map-editor-m11a.md**, **docs/map-editor-m11b.md**, **docs/Building_Cutaway_Visibility_Fix_Plan.md**, **docs/map-editor-m11c1a.md**, **docs/map-editor-m11c1b.md**, **docs/map-editor-m11c2.md**, **docs/map-editor-guide.md**.
> **Người dùng tự commit và push mọi thay đổi. Không tự commit/push. Cập nhật CURRENT_STATE cuối mỗi sprint.**

## 0. Đồ họa G0 — audit, scene chuẩn, baseline (mới nhất, chưa commit — chi tiết docs/graphics-g0.md)

Hình ảnh game, save, schema map và các world cũ không đổi.

- **Audit** (docs/graphics-g0.md mục 1): three 0.186 / R3F 9.8, WebGL2, ACES Filmic + sRGB, MSAA 4, chưa có texture ảnh nào; camera ortho zoom 28 px/m (5 cm ≈ 1,4 px); batch tĩnh = một `BatchedMesh` mỗi chunk, một material trắng tô màu theo instance, khối đơn vị co giãn (mảnh cắt lớp = ma trận thấp hơn → UV phải theo kích thước vật lý); ngoại hình tường/đồ đạc = hộp collider; mái là tấm phẳng; `indoorShading` vá `MeshStandardMaterial.prototype` (trong nhà chỉ màu nền có tác dụng; material tự gán `onBeforeCompile` sẽ thoát bản vá); nhân vật ~11 mesh + 5 material mỗi rig. **Lỗi thấy được**: camera bóng mặt trời cố định ±36 m quanh gốc (nhà B ở graphics-lab không có bóng; neighborhood-50 không bị); đường lưới 1 m trên nền đất sáng hơn đất ban đêm.
- **`content/maps/graphics-lab`** (ẩn, `?world=graphics-lab`, không đóng băng): prefab `building/lab-house` 12 × 9 m hai tầng (phòng khách, bếp, phòng ngủ; tầng trên hành lang + phòng ngủ; cầu thang), nhà A (16, 12) và nhà B cùng prefab xoay 180° ở (32, 36); đường, hai vỉa hè, lối đi, rào, bốn cây, xe, hộp thư, thùng rác. `src/map/graphicsLab.test.ts` giữ các thành phần kế hoạch yêu cầu và vị trí cảnh của script.
- **Đo**: `PerfMonitor.startFrameLog/takeFrameLog` (từng frame: interval, CPU, draw call, tam giác; tối đa 20 000); `PerfProbe` đặt gauge trước `recordFrame`; dev `window.__gl`.
- **`scripts/g0-graphics-baseline.mjs`**: 6 cảnh cố định (ngày ngoài/trong/tầng trên/toàn cảnh, đêm ngoài/trong bật đèn), 1280×800, DPR 1, 8 zombie đứng yên; frame median/p95/p99/max, CPU, draw call, tam giác, số tài nguyên, bộ nhớ ước tính, vòng đời 10 lần, tải lạnh/ấm (production: chỉ tải), `--compare` pixel. Cờ `--gpu`, `--uncapped` (chi phí thật; headless khóa nhịp ~6,1 ms), `--headed`, `--jpeg`, `--shadows`, `--dpr`.
- **Baseline** (RTX 3060, i5-11500, Chrome 154 headless, bóng high, `--uncapped`): frame median 2,6–3,6 ms, p95 5,3–10 ms, p99 ≤ 12,9 ms; draw call 60 (trong nhà) / 145 (ngoài trời, có zombie); tam giác 4,8–6,1 k; 13 texture, 57 geometry, vòng đời không tăng; production tải lạnh 290 ms / 1,96 MB, vào game 994 ms; hai lần chạy ảnh lệch 0 %. Ảnh + report ở `docs/graphics/g0-baseline/`. Chưa đo: cửa sổ thật có vsync, máy yếu, thời gian GPU tách riêng.
- **Kiểm chứng**: 569 test (+13 skip; mới `core/perf.test.ts` 2, `map/graphicsLab.test.ts` 4); lint, build, build:editor, check:bundle, map:check --deep sạch; `worlds-browser.mjs` PASS.

Commit message gợi ý: **feat(graphics): G0 audit and baseline (graphics-lab world with a two-storey model house and a turned copy, street, sidewalks, yard, trees and car; per-frame log in the perf monitor; baseline script with fixed scenes, frame time median/p95, draw calls, resource counts, memory estimate, lifecycle, load times and pixel comparison; audit and baseline report)**

## 0-M11c-2. M11c-2 — editor nhiều tầng (đã commit 3674656 — chi tiết docs/map-editor-m11c2.md)
