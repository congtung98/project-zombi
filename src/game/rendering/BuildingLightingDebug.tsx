...
export function BuildingLightingDebug() {
  ...
  useFrame(() => {
    ...
    let v = 0
    const seg = (ax: number, az: number, bx: number, bz: number, c: [number, number, number]) => {
      pos.setXYZ(v, ax, Y, az)
      col.setXYZ(v++, c[0], c[1], c[2])
      pos.setXYZ(v, bx, Y, bz)
      col.setXYZ(v++, c[0], c[1], c[2])
    }
    rooms.forEach((r, i) => {
    ...
    edges.forEach((edge, i) => {
      ...
      const state = runtime.world.doors.get(edge.doorId)?.state
      const t = doorTransmission(state, CFG)
      ...
      for (const side of [edge.a, edge.b]) {
        ...
        seg(door.center.x, door.center.z, end.x, end.z, c)
      }
  ...
  return (
    <>
      ...
      {edges.map((edge, i) => {
        const door = runtime.map.doors.find((d) => d.id === edge.doorId)!
        return (
          <Html key={edge.doorId} position={[door.center.x, 2.4, door.center.z]} center zIndexRange={[4, 0]} style={{ pointerEvents: 'none' }}>
            <div ref={(el) => { doorLabels.current[i] = el }} className="lighting-debug-label small" />
          </Html>
        )
      })}
      {windows.map((w, i) => (
        <Html key={w.id} position={[w.center.x + w.inward.x * 0.6, 2.2, w.center.z + w.inward.z * 0.6]} center zIndexRange={[4, 0]} style={{ pointerEvents: 'none' }}>
