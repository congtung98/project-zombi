...
describe('frozen test neighbourhood', () => {
  ...
  it('other worlds are the live ones (M11b: the floors lab is frozen too, M11c-1A: the cutaway lab)', () => {
    const frozenWorlds = new Set(Object.keys(FROZEN).map((p) => p.split('/')[5]))
    expect([...frozenWorlds].sort()).toEqual(['cutaway-lab', 'floors-lab', DEFAULT_WORLD_ID])
