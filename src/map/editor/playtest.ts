...
import { playAreaRect } from '../transform.ts'
...
export function playPointProblem(doc: MapDocument, p: XZ): string | null {
  const a = playAreaRect(doc.world.playArea)
  if (p.x < a.minX || p.x > a.maxX || p.z < a.minZ || p.z > a.maxZ) return `(${p.x}, ${p.z}) nằm ngoài vùng chơi`
