...
//        [--force] [--dry]
// Same seed + blocks + catalog + generator version → byte-identical files. An existing folder is
...
import { GENERATOR_NAME, GENERATOR_VERSION, generateTown, type Catalog } from '../../src/map/tools/generator.ts'
...
const doc = generateTown({ worldId, name, seed, blocksX, blocksZ }, catalog, { lootTables })
const ms = performance.now() - t0
const counts = [...doc.chunks.values()].reduce((n, c) => ({ instances: n.instances + c.instances.length, objects: n.objects + c.objects.length, spawns: n.spawns + c.spawns.length }), { instances: 0, objects: 0, spawns: 0 })
console.log(`${worldId}: seed ${seed}, ${blocksX}×${blocksZ} blocks, play area ${doc.world.playArea.size} m, ${doc.world.chunks.length} chunks, ${counts.instances} buildings, ${counts.objects} objects, ${counts.spawns} spawns (${ms.toFixed(0)} ms)`)
...
if (existsSync(join(out, 'world.json'))) {
  ...
  const again = generateTown({ worldId: existing.worldId, name: existing.name, seed: g.seed, blocksX: Number(g.params.blocksX), blocksZ: Number(g.params.blocksZ) }, catalog, { lootTables })
